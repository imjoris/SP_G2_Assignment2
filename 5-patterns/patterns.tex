...
However, the daemon was referenced quite a lot when looking at table \ref{table:deptablehighestin}, that lists the packages with the highest amount of incoming references, earlier discussed in section \ref{sec:viewlogical}.
It is also remarkable that only the daemon seems to be referencing the container, when listing the dependencies. So clearly, the docker daemon is a main access point for modifying the containers.\\
