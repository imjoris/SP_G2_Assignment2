...
available. Overlapping may also occur since multiple Docker instances may use the same image or configuration file, which is actually can be stored and organized elsewhere. Docker
...
%TODO joris: Add ref to registry
Using the Docker files and the docker commands, personalized images can be created that support a specified project. Docker allows these images to be stored in a docker registry, earlier discussed in \ref{}. Anyone with access to this registry can pull this image and use it.\\
Users of this project its application can pull the image in order to run the project and obtain the desired functionality. This registry also allows the image to be easily shared with other developers working on any related project.\\
...
\item[Source]~\\
%TODO joris: add more books/refs
...
%TODO fix, use POSA or something
\begin{quote}
...
The users of Docker want to have customization or require functionality that is not included in Docker, by extending Docker with third party custom-built tools. This customization means that third parties are able to write their own tools that extend Docker's core functionality\cite{dockerpluginblog}.

%TODO joris: add consequences of this issue (regarding key d's?).
The third party custom-built tools are only usable during runtime.
